...
export async function run({ config }) {
  ...
  if (!dbId) {
    ...
    const tomlPath = join(projectDir, "apps/worker/wrangler.toml");
  ...
  // デプロイ
  const doDeploy = await askConfirm("Cloudflare Workers にデプロイしますか？");
  let workerUrl = "";
  if (doDeploy) {
    console.log("\n  デプロイ中...\n");
    workerUrl = await deploy(join(projectDir, "apps/worker"), wranglerOpts);
    console.log("");
  }
